...
### Experimental API
When the `config.experimental` boolean flag is true, [input objects](https://spec.graphql.org/June2018/#sec-Input-Objects) are generated for node selection and node property input:
```js
config: {
  experimental: true
}
