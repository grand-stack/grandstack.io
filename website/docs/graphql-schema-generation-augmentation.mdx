...
### Experimental API
When the `config.experimental` boolean flag is set to true, the API generated is based on the mutation schema design specified by [OpenCRUD](https://www.opencrud.org/#sec-Mutations):
```js
config: {
  experimental: true
}
