...
Similar to `neo4j-graphql-js` this library focuses on query translation, just for the _JVM_ instead of Node.js.
...
But _not for nested field resolvers or data fetching_.
...
Inspired by the [graph.cool/Prisma filter approach](https://www.graph.cool/docs/reference/graphql-api/query-api-nia9nushae), our filters work the same way.
...
- a `_PersonFilter` for the `filter` argument, which is a deeply nested input object
- mutations for:
