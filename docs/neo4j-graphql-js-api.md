...
|------------|--------------------------------------------|---------|---------|------|
| `query`    | Configure the autogenerated Query fields. Can be enabled/disabled for all types or a list of individual types to exclude can be passed. Commonly used to exclude payload types.   | `Boolean` or `Object`  | `true` |   `{query: false}`<br><br> `{query:  {exclude: ["MyPayloadType"]}`     |
| `mutation` | Configure the autogenerated Mutation fields. Can be enabled/disabled for all types or a list of individual types to exclude can be passed. Commonly used to exclude payload types. |  `Boolean` or `Object`    | `true`    | `{mutation: false}` <br><br> `{query: {exclude: ["MyPayloadType"]}`|
